...
Circumvision verifies that Netlify Identity registration remains **Invite only** before admitting a confirmed account. Accounts with an explicit `circumvision`/`admin` role remain authorized, and open registration fails closed. For an additional one-user allowlist, set `CIRCUMVISION_ALLOWED_EMAILS` to Tyshone's email.
